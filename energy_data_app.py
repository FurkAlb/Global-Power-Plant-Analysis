...
if power_plants is not None:
    ...
    # Page 3: Exploratory Analysis
        ...
        if 'clean_data' not in st.session_state:
            ...
            def fill_row(row):
                gen_values = row[generation_columns].values
                valid_values = gen_values[~np.isnan(gen_values)]
                if len(valid_values) > 0:
                    mean_gen = np.mean(valid_values)
                    for col in generation_columns:
                        if np.isnan(row[col]):
        ...
        fig = px.scatter_geo(
            ...
            color='primary_fuel',
            hover_name='name',
            hover_data=['country', 'capacity_mw', 'commissioning_year', 'generation_gwh_2019'],
    ...
    # Page 4: Feature Engineering
        ...
        if 'generation_gwh_2013' in df_features.columns and 'generation_gwh_2019' in df_features.columns:
            ...
            df_features['generation_trend_pct'] = (df_features['generation_trend'] / df_features['generation_gwh_2013'] * 100)
            
            # Display generation trend
            st.dataframe(df_features[['country', 'name', 'primary_fuel', 'generation_gwh_2013', 'generation_gwh_2019', 'generation_trend', 'generation_trend_pct']].head())
            
            # Visualize generation trend by fuel type
            trend_by_fuel = df_features.groupby('primary_fuel')['generation_trend_pct'].mean().sort_values(ascending=False)
        ...
        df_features['generation_std'] = df_features[generation_columns].std(axis=1)
        df_features['generation_variability'] = df_features['generation_std'] / df_features['avg_annual_generation']
        
        # Replace infinite or NaN values with 0
        df_features['generation_variability'] = df_features['generation_variability'].replace([np.inf, -np.inf], np.nan).fillna(0)
        
        # Display generation variability
        st.dataframe(df_features[['country', 'name', 'primary_fuel', 'avg_annual_generation', 'generation_std', 'generation_variability']].head())
        
        # Visualize variability by fuel type
        variability_by_fuel = df_features.groupby('primary_fuel')['generation_variability'].mean().sort_values(ascending=False)
    ...
    # Page 5: Model Building
        ...
        target_var = st.selectbox("Select target variable to predict:", target_options, index=target_options.index('generation_gwh_2019') if 'generation_gwh_2019' in target_options else 0)
        
        # Select features for the model
        st.subheader("Feature Selection")
        
        potential_features = ['capacity_mw', 'plant_age', 'latitude', 'longitude']
        capacity_factor_cols = [col for col in df_features.columns if 'capacity_factor' in col and target_var.split('_')[-1] not in col]
        potential_features.extend(capacity_factor_cols)
        
        if 'avg_annual_generation' in df_features.columns and 'avg_annual_generation' != target_var:
            potential_features.append('avg_annual_generation')
            
        if 'generation_variability' in df_features.columns:
            potential_features.append('generation_variability')
        
        # Filter out features with high correlation to the target (like avg_annual_generation if target is a generation column)
        if 'avg_annual_generation' in potential_features and 'generation_gwh' in target_var:
            potential_features.remove('avg_annual_generation')
            
        # Let user select features
        selected_features = st.multiselect(
            "Select features for the model:", 
            potential_features,
            default=['capacity_mw', 'plant_age'] if 'plant_age' in potential_features else ['capacity_mw']
        )
        
        # Add categorical features option
        use_fuel_type = st.checkbox("Include fuel type as categorical feature", value=True)
        if use_fuel_type:
            st.info("Fuel type will be one-hot encoded for modeling.")
        ...
        if use_fuel_type:
            X = pd.get_dummies(model_df[selected_features + ['primary_fuel']], drop_first=True)
    ...
    # Page 6: Model Evaluation
        ...
        st.write("This interface allows you to input values for the model features and get a generation prediction.")
        
        # Extract top numerical features (excluding one-hot encoded ones)
        numerical_features = [f for f in model_data['feature_names'] if '_' not in f or 'capacity_factor' in f]
        
        if len(numerical_features) > 0:
            st.write("Select values for key numerical features:")
            
            # Create input for each numerical feature
            input_values = {}
            
            for feature in numerical_features[:5]:  # Limit to top 5 for simplicity
                # Get the min, max and median values from the training data
            ...
            # Add default values for other features not included in the sliders
            for feature in model_data['feature_names']:
                if feature not in input_df.columns:
                    # If it's a binary feature, default to 0
                    input_df[feature] = 0
            ...
            if st.button("Predict Generation"):
                ...
                st.success(f"Predicted {target_name}: {prediction:.2f} GWh")
                
                # Explain what the prediction means
                st.markdown(f"""
                ### Interpretation:
                
                With the given input features, the model predicts the power plant would generate **{prediction:.2f} GWh** of electricity.
                
                For comparison:
                - Median {target_name} in the dataset: {y_train.median():.2f} GWh
                - A typical plant with capacity {input_values['capacity_mw']:.2f} MW would be expected to generate around {input_values['capacity_mw'] * 8760 * 0.5 * 0.001:.2f} GWh assuming a 50% capacity factor.
                """)
else:
